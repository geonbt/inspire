...
CREATE OR REPLACE FUNCTION gn_trg_ve_node_transformer()
	RETURNS trigger
	LANGUAGE 'plpgsql'
	COST 100
	VOLATILE LEAKPROOF
...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		INSERT INTO man_transformer (node_id,power) VALUES (NEW.id,NEW.power);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		UPDATE man_transformer
		SET node_id = NEW.id, power = NEW.power
		WHERE node_id = OLD.id;
	
	ELSIF TG_OP = 'DELETE' THEN
	
		DELETE FROM node WHERE id = NEW.id;
		DELETE FROM man_transformer WHERE node_id = NEW.id;
